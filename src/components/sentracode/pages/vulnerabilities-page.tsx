...
import {
  Download, ChevronRight, Check, GitPullRequest,
  X, Loader2, RefreshCw, Filter, AlertTriangle,
  ShieldAlert, Info,
} from "lucide-react";
...
const SEV_UI: Record<Sev, { badge: "Critical" | "Warning" | "Info"; icon: React.ReactNode; label: string }> = {
  CRITICAL: { badge: "Critical", icon: <AlertTriangle className="h-3 w-3" />, label: "Critical" },
  WARNING:  { badge: "Warning",  icon: <ShieldAlert className="h-3 w-3"  />, label: "Warning"  },
  INFO:     { badge: "Info",     icon: <Info className="h-3 w-3"          />, label: "Info"     },
};

const STATUS_UI: Record<Status, "Critical" | "Warning" | "Success" | "Neutral"> = {
  OPEN:      "Critical",
  IN_REVIEW: "Warning",
  FIXED:     "Success",
  IGNORED:   "Neutral",
...
export function VulnerabilitiesPage() {
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[12px] text-white/30">
            {open} open · {criticals} critical · {warnings} warnings · {infos} info
          </p>
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          { label: `${criticals} Critical`, sev: "Critical" as const, filter: "CRITICAL" },
          { label: `${warnings} Warning`,  sev: "Warning"  as const, filter: "WARNING"  },
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-primary/25 bg-primary/8 px-4 py-2.5">
          <span className="text-[12.5px] text-primary font-medium">{selected.size} selected</span>
          <button
          ...
          >
            <Check className="h-3 w-3" />Mark all fixed
          </button>
          <button
          ...
          >
            <X className="h-3 w-3" />Ignore all
          </button>
          <button
      ...
      {loading ? (
        ...
        <Card>
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm font-medium text-white/50">No vulnerabilities found</p>
            ...
            <thead>
              <tr className="border-b border-white/5">
                ...
                {["Severity","Finding","Repository","Status","Age",""].map(h => (
                  <th key={h} className="px-3 py-3 text-left text-[10.5px] font-medium uppercase tracking-wider text-white/20">
                    {h}
                  </th>
                ))}
              </tr>
                ...
                <Fragment key={f.id}>
                  ...
                  >
                    ...
                    <td className="px-3 py-3 max-w-xs">
                      <p className="text-[12.5px] text-white/70 truncate">{f.title}</p>
                      <p className="font-mono text-[11px] text-white/25 truncate">
                        {f.filePath}{f.lineNumber ? `:${f.lineNumber}` : ""}
                        {f.cwe && <span className="ml-2 text-white/15">{f.cwe}</span>}
                      </p>
                    </td>
                    <td className="px-3 py-3 text-[12px] text-white/40">{f.repo.fullName}</td>
                    <td className="px-3 py-3">
                      <SBadge sev={STATUS_UI[f.status]}>
                        {f.status.replace("_", " ")}
                      </SBadge>
                    </td>
                    <td className="px-3 py-3 text-[11.5px] text-white/25">{timeAgo(f.createdAt)}</td>
                    <td className="px-3 py-3">
                      <ChevronRight className={`h-4 w-4 text-white/15 transition-transform ${expanded === f.id ? "rotate-90" : ""}`} />
                    </td>
                  </tr>

                  {/* expanded detail */}
                  {expanded === f.id && (
                    <tr key={`exp-${f.id}`} className="border-b border-white/5">
                      <td colSpan={7} className="px-4 pb-5 pt-2">
                        <div className="rounded-xl border border-white/5 bg-white/[0.015] p-5 space-y-4">
                          ...
                          <div>
                            ...
                            <p className="text-[12.5px] text-white/60 leading-relaxed">{f.description}</p>
                          </div>
                          ...
                          {(f.diffBefore || f.diffAfter) && (
                            <div>
                              ...
                              <DiffBlock
                                ...
                                lines={[
                                  ...(f.diffBefore ? f.diffBefore.split("\n").map(t => ({ type: "-" as const, text: t })) : []),
                                  ...(f.diffAfter  ? f.diffAfter.split("\n").map(t => ({ type: "+" as const, text: t })) : []),
                                ]}
                              />
                            </div>
                          )}
                          ...
                          {f.fix && (
                            <div>
                              <p className="mb-1 text-[10.5px] font-medium uppercase tracking-wider text-white/20">How to fix</p>
                              <p className="text-[12.5px] text-white/50 leading-relaxed">{f.fix}</p>
                            </div>
                          )}
                          ...
                          {f.snippet && !f.diffBefore && (
                            <div>
                              <p className="mb-1.5 text-[10.5px] font-medium uppercase tracking-wider text-white/20">Vulnerable snippet</p>
                          ...
                          {f.status === "OPEN" || f.status === "IN_REVIEW" ? (
                            <div className="flex gap-2 flex-wrap pt-1">
                              ...
                              >
                                {updating === f.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <GitPullRequest className="h-3.5 w-3.5" />}
                                Create PR from fix
                              </button>
                              <button
                                onClick={() => updateFinding(f.id, "FIXED")}
                              ...
                              <button
                                onClick={() => updateFinding(f.id, "IN_REVIEW")}
                                disabled={updating === f.id}
                              ...
                              <button
                                onClick={() => updateFinding(f.id, "IGNORED")}
                                disabled={updating === f.id}
                            ...
                            <div className="flex items-center gap-2">
                              <SBadge sev={STATUS_UI[f.status]}>{f.status}</SBadge>
                              <button
